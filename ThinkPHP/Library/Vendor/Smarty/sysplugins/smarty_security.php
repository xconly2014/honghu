...
/**
 * This class does contain the security_center settings
 */
class Smarty_Security {
     ...
     * {@link $security_center} is disabled during their inclusion/execution.
     *
     * @var array
     */
    ...
    public function isTrustedPhpFunction($function_name, $compiler)
    {
        ...
        $compiler->trigger_template_error("PHP function '{$function_name}' not allowed by security_center setting");
    ...
    public function isTrustedStaticClass($class_name, $compiler)
    {
        ...
        $compiler->trigger_template_error("access to static class '{$class_name}' not allowed by security_center setting");
        return false; // should not, but who knows what happens to the compiler in the future?
    }
    ...
    public function isTrustedPhpModifier($modifier_name, $compiler)
    {
        ...
        $compiler->trigger_template_error("modifier '{$modifier_name}' not allowed by security_center setting");
        return false; // should not, but who knows what happens to the compiler in the future?
    }
    ...
    public function isTrustedTag($tag_name, $compiler)
    {
        ...
        // check security_center settings
        if (empty($this->allowed_tags)) {
            if (empty($this->disabled_tags) || !in_array($tag_name, $this->disabled_tags)) {
                return true;
            } else {
                $compiler->trigger_template_error("tag '{$tag_name}' disabled by security_center setting", $compiler->lex->taglineno);
            }
        } else if (in_array($tag_name, $this->allowed_tags) && !in_array($tag_name, $this->disabled_tags)) {
            return true;
        } else {
            $compiler->trigger_template_error("tag '{$tag_name}' not allowed by security_center setting", $compiler->lex->taglineno);
        }
        return false; // should not, but who knows what happens to the compiler in the future?
    }
    ...
    public function isTrustedModifier($modifier_name, $compiler)
    {
        ...
        if (empty($this->allowed_modifiers)) {
            if (empty($this->disabled_modifiers) || !in_array($modifier_name, $this->disabled_modifiers)) {
                ...
                $compiler->trigger_template_error("modifier '{$modifier_name}' disabled by security_center setting", $compiler->lex->taglineno);
            }
        } else if (in_array($modifier_name, $this->allowed_modifiers) && !in_array($modifier_name, $this->disabled_modifiers)) {
            return true;
        } else {
            $compiler->trigger_template_error("modifier '{$modifier_name}' not allowed by security_center setting", $compiler->lex->taglineno);
        }
        return false; // should not, but who knows what happens to the compiler in the future?
    }
    ...
    public function isTrustedStream($stream_name)
    {
        ...
        throw new SmartyException("stream '{$stream_name}' not allowed by security_center setting");
    }
    ...
    public function isTrustedResourceDir($filepath)
    {
        ...
        // give up
        throw new SmartyException("directory '{$_filepath}' not allowed by security_center setting");
    }
    ...
    public function isTrustedPHPDir($filepath)
    {
        if (empty($this->trusted_dir)) {
            throw new SmartyException("directory '{$filepath}' not allowed by security_center setting (no trusted_dir specified)");
        }

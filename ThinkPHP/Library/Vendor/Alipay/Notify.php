...
class AlipayNotify {
    ...
    function verifyNotifyMobile(){
        if(empty($_POST)) {//判断POST来的数组是否为空
            ...
            //生成签名结果
            $isSign = $this->getSignVeryfyMobile($decrypt_post_para, $_POST["sign"],false);
    ...
    function verifyReturnMobile(){
        if(empty($_GET)) {//判断GET来的数组是否为空
            return false;
        }
        else {
            //生成签名结果
            $isSign = $this->getSignVeryfyMobile($_GET, $_GET["sign"],true);
     ...
     * @return 签名验证结果
     */
    function getSignVeryfyMobile($para_temp, $sign, $isSort) {
        //除去待签名参数数组中的空值和签名参数

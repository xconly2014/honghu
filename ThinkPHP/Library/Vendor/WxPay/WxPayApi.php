...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid(C('wx_config.app_id'));//公众账号ID
	...
	public static function notify($callback, &$msg)
	{
		...
		try {
			...
			Think\Log::write("try end");
		} catch (WxPayException $e){
			$msg = $e->errorMessage();
			return false;
		}

		Think\Log::write("xml end");
	...
	{
		//如果不需要上报数据
		if(C('wx_config.report_level') == 0){
			return;
		} 
		//如果仅失败上报
		if(C('wx_config.report_level') == 1 &&
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, C('wx_config.ssl_cert_path'));
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, C('wx_config.ssl_key_path'));
		}
		//post提交方式
